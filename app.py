...
def get_best_teacher():
    ...
    matching_teachers = [teacher for teacher in teachers if topic in teacher["expertise"].lower() and learning_mode.lower() == teacher["learning_mode"].lower()]
    
    if matching_teachers:
        best_teachers = [teacher["name"] for teacher in matching_teachers]
        print("Best Teachers:", best_teachers)  # Add this print statement
        return jsonify({"best_teachers": best_teachers})
    else:
        return jsonify({"best_teachers": "No teachers found for this topic and learning mode."})
